...
export default function App() {
  ...
  const usersList = [
    ...
    { name: "jeric bonifar", handle: "@jeric_bonifar@gmail.com", role: "clerk" },
    { name: "System Administrators", handle: "@admin_root", role: "admin" },
  ];
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      ...
      if (res.ok) { setIsDeleteModalOpen(false); setProductToDelete(null); fetchProducts(); }
    } catch (err) { console.error(err); }
  };
                ...
                {filteredProducts.map(item => (
                  <tr key={item.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="p-6 text-right space-x-2">
                      <button onClick={() => { setSelectedProduct(item); setIsEditModalOpen(true); }} className="p-2 bg-slate-800 rounded-lg hover:bg-[#4361ee] text-white transition-colors">✏️</button>
      ...
      {isDeleteModalOpen && productToDelete && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-6">
          <div className="bg-[#111827] border border-slate-800 w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl text-center">
            <div className="w-16 h-16 bg-red-500/10 text-red-500 rounded-2xl flex items-center justify-center mx-auto mb-6 text-3xl">🗑️</div>
            <h2 className="text-xl font-black text-white mb-2">Delete Product?</h2>
            <p className="text-slate-400 text-sm mb-8">Are you sure you want to delete <span className="text-white font-bold">{productToDelete.name}</span>? This action cannot be undone.</p>
            <div className="space-y-3">
              <button onClick={handleDeleteProduct} className="w-full py-4 bg-red-500 rounded-2xl font-bold text-white shadow-lg hover:bg-red-400 transition-all">Confirm Delete</button>
              <button onClick={() => setIsDeleteModalOpen(false)} className="w-full py-2 text-slate-500 font-bold">Cancel</button>
            </div>
          </div>
        </div>
      )}
